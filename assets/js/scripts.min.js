...
var options={valueNames:["name","tagline","last_name_sort","genre"],listClass:"list-filter",page:1e3},countryList=new List("container-filter",options);
jQuery(function(){function n(n){var t=$("#search_results");window.data.then(function(i){n.length?(t.empty(),n.forEach(function(n){var e=i[n.ref],o="<li><small>"+e.collection+'</small><br /><a href="'+e.url+'">'+e.title+"</a></li>";t.append(o)})):t.html("<li><strong>No results found</strong></li>")})}window.idx=lunr(function(){this.field("id"),this.field("title",{boost:10}),this.field("collection"),this.field("summary")}),window.data=$.getJSON("/content.json"),window.data.then(function(n){$.each(n,function(n,t){window.idx.add($.extend({id:n},t))})}),$("#search_box").on("click",function(){$("#search_results").toggleClass("show")}),$("#site_search").bind("keyup",function(){event.preventDefault();var t=$("#search_box").val(),i=window.idx.search(t);n(i)})});
function parallax(){if($("#js-parallax-window").length>0){var n=$("#js-parallax-background"),e=$("#js-parallax-window"),t=$(e).offset().top,i=$(window).scrollTop(),o=t-i,a=($(n).offset().top,window.innerHeight,.35);n.css("top",-(o*a)+"px")}}$(document).ready(function(){$("#js-parallax-window").length&&parallax()}),$(window).scroll(function(n){$("#js-parallax-window").length&&parallax()});var menuToggle=$("#js-mobile-menu").unbind();$("#js-navigation-menu").removeClass("show"),menuToggle.on("click",function(n){n.preventDefault(),$("#js-navigation-menu").slideToggle(function(){$("#js-navigation-menu").is(":hidden")&&$("#js-navigation-menu").removeAttr("style")})});var sidebarToggle=$("#js-mobile-menu-sidebar").unbind();$("#js-navigation-menu").removeClass("show"),menuToggle.on("click",function(n){n.preventDefault(),$("#js-navigation-menu-sidebar").slideToggle(function(){$("#js-navigation-menu-sidebar").is(":hidden")&&$("#js-navigation-menu-sidebar").removeAttr("style")})}),$(".sliding-panel-button").on("click touchstart",function(n){$(".sliding-panel-content, .content, .open, .close").toggleClass("is-visible"),n.preventDefault(),console.log("working")}),$(".js-accordion-trigger").bind("click",function(n){jQuery(this).parent().find(".submenu").slideToggle("fast"),jQuery(this).parent().toggleClass("is-expanded"),n.preventDefault()}),$(".js-accordion-trigger-footnotes").bind("click",function(n){jQuery(this).parent().find(".footnotes").slideToggle("fast"),jQuery(this).parent().toggleClass("is-expanded"),n.preventDefault()}),$('a:not([href^="http://www.aba-icc.org/"]):not([href^="#"]):not([href^="/"])').attr("target","_blank"),function(n){n.mark={jump:function(e){var t={selector:".nav-link-internal a"};return"string"==typeof e&&(t.selector=e),e=n.extend(t,e),n(e.selector).click(function(e){var t=n(this),i=t.attr("href"),o=1e3,a=n(i).offset().top;n("html,body").animate({scrollTop:a},o,"swing"),e.preventDefault()})}}}(jQuery),jQuery(function(){jQuery.mark.jump()});var sticky=new Waypoint.Sticky({element:$(".fixed-nav")[0]});jQuery(document).ready(function(n){n("#main").waypoint(function(e){"down"===e?n(".fixed-list").toggleClass("stuck"):n(".fixed-list").removeClass("stuck")},{offset:"65%"})});